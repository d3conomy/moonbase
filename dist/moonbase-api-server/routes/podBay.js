...
import { execute } from '../../moonbase-interfaces/command.js';
...
router.post('/pods', async function (req, res) {
    ...
    const idRef = new IdReference({ component: Component.POD, id, type: podBay.options.nameType });
 ...
 *     example: /or
 * */
router.delete('/pods', async function (req, res) {
    const podBay = req.podBay;
 ...
 *     example: /or
 * */
router.get('/pod/:id', timeout(timeoutDuration), async function (req, res, next) {
    const podBay = req.podBay;
    ...
    try {
        if (command) {
            result = await execute({ pod, command: command });
        }
        else {
 ...
 *     example: /or
 * */
router.post('/pod/:id', timeout(timeoutDuration), async function (req, res, next) {
    const podBay = req.podBay;
    const podId = req.params.id;
 ...
 *     example: /or
 * */
router.put('/pod/:id', async function (req, res) {
    const podBay = req.podBay;
    const podId = req.params.id;
